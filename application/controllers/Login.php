			...
			if( $cek->num_rows() != 1){
				...
				if(password_verify($password,$isi->password) === TRUE){
					$data_session = array(
									...
									'status' => 'login',
									'role' => $isi->role,
									'pilih_rt' => $isi->pilih_rt,
									'pilih_rw' => $isi->pilih_rw,
									'dapil' => $isi->dapil,
					...
					$this->M_login->edit_user(['username' => $username],['last_login' => date('d-m-Y G:i')]);
						...
						}else if($isi->role == 0 && $isi->pilih_rw == 'belum' && $isi->pilih_rt == 'belum'){
							redirect(base_url('pemilihan'));
						}
						else{
							$this->session->set_flashdata('msg','Anda Sudah Menggunakan Hak Pilih');
							redirect(base_url());
						}
				}else {
